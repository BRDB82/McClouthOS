...
if [ "$1" == "--update" ]; then
        curl -fsSL "https://raw.githubusercontent.com/BRDB82/McClouthOS/main/RHEL/test/si2-test.sh" -o "si2-test.new" || {
                echo "update failed"
            rm "si2-test.new"
            exit 1
        }
	chmod +x "si2-test.new"
        mv -f "si2-test.new" "si2-test.sh"
        echo ""
        echo "Update installed. Please restart si2-test.sh."
        exit 0
fi
